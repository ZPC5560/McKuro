...
| 启动器 | 检查更新(版本数值比较)、**预下载**(不影响已安装文件,支持暂停/继续)、安装更新、**修复游戏**(跳过校验文件)、**下载速度限制**(MB/s)、**启动参数**(DX11/DLSS/自定义参数/可选启动文件)、**DLSS/XeSS 版本检测**、打开游戏目录;选目录后自动识别加载;官方封面轮播 + 游戏公告(带封面)+ **背景视频封面**(可选,libmpv **GPU 渲染**(OpenGL+硬件解码,CPU 占用约为软件渲染的 1/6;GL 不可用自动回退软件渲染,无 native 库回退官方首帧图);可**自定义动态壁纸**:本地视频或**自动扫描 Wallpaper Engine 视频壁纸**含封面预览(自定义生效时自动隐藏官方版本诗句叠加,窗口最小化/游戏进行中自动挂起让出资源);**启动按钮三态(启动游戏/启动中/游戏中)+ 进程监控**;**启动后最小化位置可选任务栏/系统托盘**;**游戏结束后窗口状态可配**;主窗口纵横比跟随启动页视频,拖动全程锁比例 |
...
| 兑换码 | 远程拉取鸣潮兑换码清单(国服/国际服,有效在前),一键复制;**英文界面按官方译名术语表展示接口内容**(星声→Astrite 等,未知条目保留原文) |
...
| 设置 | 游戏目录(自动识别加载)、服务器渠道、游戏修复·跳过校验文件管理、下载并发/限速、游戏启动参数、**启动后最小化位置(任务栏/托盘)**、**游戏结束后窗口状态**、**启动时打开的页面(主页/启动页)**、主题/背景视频封面(**视频来源可切换:官方宣传视频 / 自定义动态壁纸**——选择本地视频文件或指向 Wallpaper Engine 内容目录自动扫描,封面网格预览点选,设置页实时预览)、**应用自更新**(GitHub Release 检查/跳过版本;**优先 zip 绿色包解压替换,exe 安装包静默自动更新**)、**界面语言 zh-Hans/en-US 全量切换**(711 key 双语资源覆盖全部页面与动态消息,重启生效) |
...
### Live2D 模型(仅 Windows x64)

设置页「Live2D 模型」区块:

1. **放置 Cubism 运行时**:`Live2DCubismCore.dll` 有 Live2D 独立许可,不随安装包分发——从 [Live2D 官网](https://www.live2d.com/sdk/download/native/) 下载 **Cubism SDK for Native**,把其中的 `Live2DCubismCore.dll` 放入「打开模型目录」按钮打开的目录(`%AppData%/McKuro/live2d`)或程序安装目录;状态徽标显示"已就绪"即可
2. **导入模型**:点「导入模型文件夹」选择包含 Cubism 3+ `.model3.json` 的文件夹(递归扫描),下拉选择模型
3. **配置**:缩放(0.5~3x)/水平/垂直位置/不透明度滑条,设置页预览可拖动与滚轮缩放并实时写回,首页即时应用
4. 开启「首页显示 Live2D 模型」开关;模型叠加在首页壁纸上方、内容卡片下层(纯展示,不挡交互)

macOS/Linux 暂不支持(渲染控件当前仅面向 Windows x64),设置页相应灰显;Cubism Core 与模型文件有各自的许可协议,请自行确认使用范围。
