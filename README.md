...
| 启动器 | 检查更新(版本数值比较)、**预下载**(不影响已安装文件,支持暂停/继续)、安装更新、**修复游戏**(跳过校验文件)、**下载速度限制**(MB/s)、**启动参数**(DX11/DLSS/自定义参数/可选启动文件)、**DLSS/XeSS 版本检测**、打开游戏目录;选目录后自动识别加载;官方封面轮播 + 游戏公告(带封面)+ **背景视频封面**(可选,libmpv 软件渲染,无 native 库自动回退官方首帧图;可**自定义动态壁纸**:本地视频或**自动扫描 Wallpaper Engine 视频壁纸**含封面预览,窗口最小化/游戏进行中自动挂起让出资源);**启动按钮三态(启动游戏/启动中/游戏中)+ 进程监控**;**启动后最小化位置可选任务栏/系统托盘**;**游戏结束后窗口状态可配**;主窗口纵横比跟随启动页视频,拖动全程锁比例 |
...
| 设置 | 游戏目录(自动识别加载)、服务器渠道、游戏修复·跳过校验文件管理、下载并发/限速、游戏启动参数、**启动后最小化位置(任务栏/托盘)**、**游戏结束后窗口状态**、**启动时打开的页面(主页/启动页)**、主题/背景视频封面(**视频来源可切换:官方宣传视频 / 自定义动态壁纸**——选择本地视频文件或指向 Wallpaper Engine 内容目录自动扫描,封面网格预览点选,设置页实时预览)、**应用自更新**(GitHub Release 检查/跳过版本;**优先 zip 绿色包解压替换,exe 安装包静默自动更新**)、界面语言(zh-Hans/en-US) |
