...
| 设置 | 游戏目录(自动识别加载)、服务器渠道、游戏修复·跳过校验文件管理、下载并发/限速、游戏启动参数、**启动后最小化位置(任务栏/托盘)**、**游戏结束后窗口状态**、**启动时打开的页面(主页/启动页)**、主题/背景视频封面、**应用自更新**(GitHub Release 检查/跳过版本;**优先 zip 绿色包解压替换,exe 安装包兼容**)、界面语言(zh-Hans/en-US) |
