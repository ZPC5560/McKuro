...
基于 **.NET 10 + Avalonia 12 + Semi Design** 的《鸣潮》(Wuthering Waves)桌面启动器,支持原生 AOT 发布。当前版本 **1.0.0**。
...
| 系统材质外观 | 导航栏/内容背景跟随操作系统原生桌面材质:**Windows 11 Mica / Windows 10 Acrylic / macOS 毛玻璃·液态玻璃 / Linux 透明背景+应用染色**;主题切换(跟随系统/浅色/深色,即时生效) |
...
| 签到 | 一键游戏签到(鸣潮全部角色)、签到奖励与签到统计、每日 8:00 自动签到 |
...
| 账号 | **全部接口账号登录的统一入口**:库街区多账号(Token/短信+极验登录/切换/移除)、云鸣潮登录、mcguide 官方评级登录;**同一账号自动判定**(手机号比对,仅提醒不强制登出) |
| 设置 | 游戏目录(自动识别加载)、服务器渠道、游戏修复·跳过校验文件管理、下载并发/限速、游戏启动参数、**启动后最小化位置(任务栏/托盘)**、**游戏结束后窗口状态**、主题/背景视频封面、**应用自更新**(GitHub Release 检查/跳过版本;**优先 zip 绿色包解压替换,exe 安装包兼容**)、界面语言(zh-Hans/en-US) |
...
│   ├── Controls/              # AsyncImage / VideoBackgroundControl(libmpv) / TimeLineChart / SpeedTrendChart / RingProgress / LruCache
...
**应用自更新**:设置页检查 GitHub Release(默认仓库 `ZPC5560/McKuro`),**优先下载 zip 绿色包**(解压替换安装目录并重启),exe 安装包需管理员启动兼容;支持跳过指定版本。
...
所有接口账号(库街区/云鸣潮/mcguide)统一在**「账号」页**登录与管理:支持 Token、手机号+验证码(极验人机验证)等方式;已登录多个接口时自动按手机号判定是否同一账号(仅提醒,不强制登出)。
