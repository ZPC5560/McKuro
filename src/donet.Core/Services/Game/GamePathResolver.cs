...
public sealed class GamePathResolver
{
    ...
    public GamePathResolver(Func<string?> rootDirGetter)
    {
    ...
    public bool IsGameInstalled => GameRootDir is not null && File.Exists(Path.Combine(GameRootDir, ExeRootName));

    public string? ClientExePath =>
        GameRootDir is null ? null : Path.Combine(GameRootDir, ExeClientRelative);

    public string? RootExePath =>
        GameRootDir is null ? null : Path.Combine(GameRootDir, ExeRootName);
